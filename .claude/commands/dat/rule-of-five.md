...
**Task size determines pass count:**

- Small: Single file, <100 lines, straightforward scope → 2-3 passes
- Large: Multi-file, complex logic, architectural changes → 4-5 passes
...
- This skill uses 4-5x more tokens than single-pass work (multiple review iterations)
- Quality improvement justifies the cost for non-trivial tasks
- Apply selectively to work that matters
...
**Declare convergence:**
Explicitly state "This is about as good as we can make it" before claiming completion.
...
**This skill orchestrates:** You apply these skills during appropriate review passes. Don't duplicate their checks - reference and use them.
...
| Excuse | Reality |
| --- | --- |
| "First draft is clean" | First drafts always have issues. Review finds them. |
| "One review is enough" | Research shows 4-5 passes reach convergence. One pass misses issues. |
| "I'm confident it's good" | Confidence ≠ quality. Reviews prove quality. |
| "Reviews waste time" | Reviews save rework time. Finding issues early is faster. |
| "This is too simple to review" | Simple code has issues too. Review takes minutes. |
| "Already behind schedule" | Shipping low-quality work creates more delays fixing it. |
| "User doesn't need perfection" | Convergent quality ≠ perfection. It's professional baseline. |
| "Not all perspectives apply" | At minimum: engineer, tester, architect apply to all code. |
...
From Jeffrey Emanuel's research:

- 4-5 iterations typically needed for convergence
- First-draft quality consistently insufficient
- Multi-pass review catches issues single review misses
- Quality threshold where "good enough" actually means good enough

**Without this skill:** First-draft quality, multiple user-driven rework cycles, low trust.

**With this skill:** Convergent quality, minimal rework, high trust threshold.
