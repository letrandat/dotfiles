...
2. When you think a task is done:
   - Report what you completed
   - Ask: "Should I close task plan-abc?"
   - Wait for user confirmation
3. Only after user says "yes" or "close it": `bd close plan-abc`

**Example:**
```
Agent: "I've implemented user authentication with JWT. All tests pass.
        Should I close task plan-abc?"
User: "Yes, close it"
Agent: [runs bd close plan-abc]
```
...
bd list              # All tasks
bd ready             # Ready to work
bd show plan-abc     # Task details

# Create with description
bd create "Title" -p 1 --description "Detailed description"

# Add/update description
bd update plan-abc --description "Updated description"

# Add comments (for updates/discussion)
bd comment plan-abc "Additional context or progress update"

# Update status (allowed)
bd update plan-abc --status in_progress

# Close task (ASK FIRST!)
bd close plan-abc --reason "Completed: implemented auth with tests"
```
...
**DO use bd when:**
- User explicitly asks to create/manage tasks
- User references task IDs (plan-abc, dotfiles-xyz)
- User asks "what's ready?" or "show my tasks"
